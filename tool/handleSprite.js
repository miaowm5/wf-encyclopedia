// 使用nodejs进行文件的处理：
// 1. 读取 assets/character 目录下的子目录，若该子目录的子目录下不包含UI目录，则跳过处理
// 2. 对于包含UI文件的子目录，若包含 square_0.png，则将其复制到 output/head 目录下，文件名为 原始目录名.png
// 3. 对于包含UI文件的子目录，若包含 full_shot_1440_1920_0.png，则将其复制到 output/full_shot 目录下，文件名为 原始目录名_0.png
// 4. 对于包含UI文件的子目录，若包含 full_shot_1440_1920_1.png，则将其复制到 output/full_shot 目录下，文件名为 原始目录名_1.png
// 5. 对于包含UI文件的子目录，若包含 story 目录，则将该目录及目录下全部文件复制到 output/story 目录下，复制后的目录名为 子目录名
...
fs.readdirSync(characterDir, { withFileTypes: true }).forEach(dirent => {
    ...
    if (!fs.existsSync(uiDirPath) || !fs.statSync(uiDirPath).isDirectory()) return;
